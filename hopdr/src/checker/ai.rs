use crate::formula::{Ident, Constraint, PredKind, Op}
...
pub fn gen_bound(x: Ident, c: &Constraint) ->  Domain {
  match c.kind() {
    crate::formula::ConstraintExpr::True |
    crate::formula::ConstraintExpr::False => Domain::all(),
    ...
    crate::formula::ConstraintExpr::Quantifier(_, _, c) => {
      gen_bound(x, c)
    }
}
}